...
|--------|-----------------|
| `HOSTINGER_HOST` | SSH hostname or IP (repo secret only — never commit) |
| `HOSTINGER_PORT` | SSH port (repo secret only) |
| `HOSTINGER_USER` | SSH username (repo secret only) |
| `HOSTINGER_SSH_KEY` | Full private key PEM (same key that SSHs to Hostinger) |
| `HOSTINGER_REMOTE_PATH` | Absolute docroot path ending in `/` (repo secret only; e.g. `domains/YOUR_DOMAIN/public_html/`) |
